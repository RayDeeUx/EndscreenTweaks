		...
		if (customLCTMode == "Combined" || customLCTMode == "Oxygene One") {
			...
			*/
			...
			if (replacementLabelText == "DOWNWELL-0 CLEARED") {
				manager->sharedReplacementLabel = fmt::format("{}-{} CLEARED", grabRandomString(manager->downwellStages), utils::numToString(getRandInt()));
				...
				manager->sharedReplacementLabel = fmt::format("{} MADE IT THROUGH ACT {}", sonicName, utils::numToString(getRandInt(7)));
			} else if (replacementLabelText == "U RAPPIN' RESULT!") {
				manager->sharedReplacementLabel = fmt::format("U RAPPIN' {}!", grabRandomString(manager->parappaResults));
			} else if (replacementLabelText == "A KNOCKOUT!" && level->m_stars.value() != 0) {
				manager->sharedReplacementLabel = grabRandomString(manager->cupheadResults);
