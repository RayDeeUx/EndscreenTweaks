...
class $modify(MyEndLevelLayer, EndLevelLayer) {
	...
	void applyTotalAttemptAndJumpCount(GJGameLevel* level) {
		...
		constexpr bool timAppleFuckingSucks = GEODE_IOS(true) GEODE_ANDROID(false) GEODE_DESKTOP(false);

		const std::string& totalAttemptsString = getModBool("totalAttemptsAndJumpsCommas") ? (timAppleFuckingSucks ? GameToolbox::pointsToString(level->m_attempts.value()) : fmt::format(std::locale("en_US.UTF-8"), "{:L}", level->m_attempts.value())) : geode::utils::numToString(level->m_attempts.value());
		const std::string& totalJumpsString = getModBool("totalAttemptsAndJumpsCommas") ? (timAppleFuckingSucks ? GameToolbox::pointsToString(level->m_jumps.value()) : fmt::format(std::locale("en_US.UTF-8"), "{:L}", level->m_jumps.value())) : geode::utils::numToString(level->m_jumps.value());
		if (showTotalAttemptsAndJumps == "fully replace") {
			attemptLabel->setString(fmt::format("Total Attempts: {}", totalAttemptsString).c_str());
			jumpsLabel->setString(fmt::format("Total Jumps: {}", totalJumpsString).c_str());
